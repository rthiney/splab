...
export class LetterAvatarDirective implements OnInit, OnChanges {
  ...
  generateLetter() {
    ...
    this.avatarData.text=this.avatarText;
    var size =
      this.avatarData && this.avatarData.size ? this.avatarData.size : 100;
    this.fontColor = this.avatarData.fontColor
      ? this.avatarData.fontColor
      : "#FFFFFF";
    var isSquare = this.avatarData && this.avatarData.isSquare ? true : false;
    var border =
      this.avatarData && this.avatarData.border
        ? this.avatarData.border
        : "1px solid #d3d3d3";
    var background =
      this.avatarData && this.avatarData.background
        ? this.avatarData.background
        : null;
    var text =
      this.avatarData && this.avatarData.text ? this.avatarData.text : null;
    ...
    var letter =
      textArray[0].substr(0, 1) +
      "" +
      (textArray.length > 1 ? textArray[1].substr(0, 1) : "");
